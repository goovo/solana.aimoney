  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="CreateTime" prop="createdAt" width="180">
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
</el-table-column>
           
            <el-table-column align="left" label="策略名" prop="name" width="220" />

            <el-table-column align="left" label="文件名" prop="fileName" width="280" />

            <el-table-column align="left" label="状态" prop="status" width="120" />

            <el-table-column align="left" label="超参优化" prop="hyperopt" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.hyperopt) }}</template>
</el-table-column>
            <el-table-column align="left" label="Buy参数个数" prop="buyParams" width="120" />

            <el-table-column align="left" label="Sell参数个数" prop="sellParams" width="120" />

            <el-table-column align="left" label="AI优化" prop="ai" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.ai) }}</template>
</el-table-column>
            <el-table-column align="left" label="K线周期" prop="timeFrame" width="120" />

            <el-table-column align="left" label="方向" prop="direction" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" style="display: none;"  @click="updateSysFreqStrategyFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.status }}
                    ...
                    <el-descriptions-item label="是否支持超参优化">
    {{ detailForm.hyperopt }}
</el-descriptions-item>
                    <el-descriptions-item label="Buy参数个数">
    {{ detailForm.buyParams }}
</el-descriptions-item>
                    <el-descriptions-item label="Sell参数个数">
    {{ detailForm.sellParams }}
</el-descriptions-item>
                    <el-descriptions-item label="是否支持AI优化">
    {{ detailForm.ai }}
</el-descriptions-item>
                    <el-descriptions-item label="K线周期">
    {{ detailForm.timeFrame }}
</el-descriptions-item>
                    <el-descriptions-item label="方向 more/less">
    {{ detailForm.direction }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
import {
  ...
  deleteSysFreqStrategy,
  deleteSysFreqStrategyByIds,
  updateSysFreqStrategy,
  findSysFreqStrategy,
  getSysFreqStrategyList
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    if (!valid) return
    page.value = 1
    if (searchInfo.value.hyperopt === ""){
        searchInfo.value.hyperopt=null
    }
    if (searchInfo.value.ai === ""){
        searchInfo.value.ai=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getSysFreqStrategyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysFreqStrategyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysFreqStrategyFunc = async(row) => {
    const res = await findSysFreqStrategy({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysFreqStrategyFunc = async (row) => {
    const res = await deleteSysFreqStrategy({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
