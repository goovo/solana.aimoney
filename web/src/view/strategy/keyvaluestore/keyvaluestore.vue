  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="进程ID" prop="pid" width="120" />

            <el-table-column align="left" label="用户" prop="key" width="120" />

            <el-table-column align="left" label="Status"   width="120" />
                 

            <el-table-column align="left" label="配置文件" prop="configFile" width="220" />

            <el-table-column align="left" label="类型"   width="180"> 
              <template #default="{ row }">
                <span v-if="row?.dryRun === '--dry-run'">虚拟盘</span>
                <span v-else>实盘</span>
              </template>
        </el-table-column>
            <el-table-column align="left" label="策略名称" prop="strategyName" width="220" />
 

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope"> 
            <el-button   type="primary" link icon="delete" @click="deleteRow(scope.row)">强行终止</el-button>
            </template>
        </el-table-column>
        </el-table>
    ...
    {{ detailForm.key }}
                    ...
                    <el-descriptions-item label="valueType字段">
    {{ detailForm.valueType }}
</el-descriptions-item>
                    <el-descriptions-item label="stringValue字段">
    {{ detailForm.stringValue }}
</el-descriptions-item>
                    <el-descriptions-item label="datetimeValue字段">
    {{ detailForm.datetimeValue }}
</el-descriptions-item>
                    <el-descriptions-item label="floatValue字段">
    {{ detailForm.floatValue }}
</el-descriptions-item>
                    <el-descriptions-item label="intValue字段">
    {{ detailForm.intValue }}
</el-descriptions-item>
            </el-descriptions>
        </el-drawer>
...
const getTableData = async() => {
  const table = await getKeyvaluestoreList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteKeyvaluestoreByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateKeyvaluestoreFunc = async(row) => {
    const res = await findKeyvaluestore({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteKeyvaluestoreFunc = async (row) => {
    const res = await deleteKeyvaluestore({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
